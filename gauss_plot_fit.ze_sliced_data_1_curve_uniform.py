...
sliced_data = np.array([i for i in data if i > 6897 and i < 6909])
...
#Transforming to unifrom distribution
mean = 6902.894
std = 4.077
uniform = np.array([st.norm.cdf((i-mean)/std) for i in sliced_data])

# Plot the Uniform Histogram
n_bins=60
plt.hist(uniform, bins=n_bins, density=True, alpha=0.6, color='turquoise', label=f"Histogram ({n_bins} bins)" )

# Display the plot
plt.xlabel('Generated Numbers')
plt.ylabel('Density')
plt.legend()
plt.title('Uniform Distribution based on Voltage Values')
plt.show()
